...
public:
              ...
              StackVariables & stack ) const
  {
    ...
    stack.localRow = m_dofNumber[ei] - m_rankOffset + roffset::MASSBAL;
    for( integer idof = 0; idof < numDof; ++idof )
    {
      stack.dofIndices[idof] = m_dofNumber[ei] + coffset::DPRES + idof;
    }
  }
  ...
  void computeAccumulation( localIndex const ei,
                            ...
                            FUNC && phaseAmountKernelOp = CompositionalMultiphaseBaseKernels::NoOpFunc{} ) const
  {
    ...
    real64 const volume = m_volume[ei];
  ...
  void computeVolumeBalance( localIndex const ei,
                             StackVariables & stack,
                             FUNC && phaseVolFractionSumKernelOp = CompositionalMultiphaseBaseKernels::NoOpFunc{} ) const
  {
    arraySlice1d< real64 const, compflow::USD_PHASE - 1 > phaseVolFrac = m_phaseVolFrac[ei];
    arraySlice1d< real64 const, compflow::USD_PHASE - 1 > dPhaseVolFrac_dPres = m_dPhaseVolFrac_dPres[ei];
    arraySlice2d< real64 const, compflow::USD_PHASE_DC - 1 > dPhaseVolFrac_dCompDens = m_dPhaseVolFrac_dCompDens[ei];
    ...
    stack.localResidual[numComp] = volume * oneMinusPhaseVolFracSum;
    for( integer idof = 0; idof < numComp+1; ++idof )
    {
      stack.localJacobian[numComp][idof] *= volume;
    }
  ...
  /**
   * @brief Performs the kernel launch, and skip the accumulation term
   * @tparam POLICY the policy used in the RAJA kernels
   * @tparam KERNEL_TYPE the kernel type
   * @param[in] numElems the number of elements
   * @param[inout] kernelComponent the kernel component providing access to setup/compute/complete functions and stack variables
  ...
  template< typename POLICY, typename KERNEL_TYPE >
  static void
  launchWithoutAccumulation( localIndex const numElems,
                             KERNEL_TYPE const & kernelComponent )
  {
    GEOSX_MARK_FUNCTION;
...
public:
   ...
   * @param[in] rankOffset the offset of my MPI rank
   * @param[in] dofKey the string key to retrieve the degress of freedom numbers
   * @param[in] skipAccumulation skip the computation of the accumulation term
   * @param[in] subRegion the element subregion
   * @param[in] fluid the fluid model
   * @param[inout] localMatrix the local CRS matrix
   * @param[inout] localRhs the local right-hand side vector
                   ...
                   arrayView1d< real64 > const & localRhs )
  {
    ...
    CompositionalMultiphaseBaseKernels::internal::kernelLaunchSelectorCompSwitch( numComps, [&] ( auto NC )
    {
      ...
      kernel( numPhases, rankOffset, dofKey, subRegion, fluid, localMatrix, localRhs );
      if( skipAccumulation )
      {
        ElementBasedAssemblyKernel< NUM_COMP, NUM_DOF >::template
        launchWithoutAccumulation< POLICY, ElementBasedAssemblyKernel< NUM_COMP, NUM_DOF > >( subRegion.size(), kernel );
      }
      else
      {
        ElementBasedAssemblyKernel< NUM_COMP, NUM_DOF >::template
